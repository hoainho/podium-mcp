...
type TransportFactory = (host: string, port: number) => Promise<EngineTransport>;

let transportFactory: TransportFactory = (host, port) => createEngineTransport(host, port);

/** Test seam: override the transport factory (null restores the real WebSocket one). */
export function _setEngineTransportFactory(factory: TransportFactory | null): void {
  transportFactory = factory ?? ((host, port) => createEngineTransport(host, port));
}

const INSTRUMENT_NOTE =
  " Requires an AltTester-instrumented build (dev/staging) with the in-app server running — " +
...
async function connectEngine(udid: string): Promise<EngineClient> {
  ...
  if (!forwarded) {
    throw new EngineError(
      `could not forward the engine port for ${udid} (adb forward / iproxy failed). Is the device connected?`
    );
  }
  ...
  try {
    ...
    throw new EngineError(
      `no AltTester server reachable on ${udid}. The app under test must be an AltTester-instrumented build with the server running.${
...
function engineErrorResult(e: unknown) {
  if (e instanceof EngineError) return errorResult(e.message);
...
export function registerEngineTools(server: McpServer): void {
  server.tool(
    "engine_inspect",
    "Lists game-engine (Unity/GL) objects matching a selector, each with absolute screen coordinates for tapping — DOM-like addressing with NO vision." +
      INSTRUMENT_NOTE,
    {
    ...
    async ({ udid, by, value }) => {
      ...
      try {
        ...
        const objects = await client.findObjects((by ?? "name") as EngineBy, value);
  ...
  server.tool(
    "engine_tap",
    "Taps a game-engine object resolved by selector (engine-reported screen coords, no vision)." + INSTRUMENT_NOTE,
    {
      udid: z.string().describe("Device UDID / Android serial"),
      by: BY.optional(),
    ...
    async ({ udid, by, value }) => {
      ...
      try {
        client = await connectEngine(udid);
        const objects = await client.findObjects((by ?? "name") as EngineBy, value);
        if (objects.length === 0) return errorResult(`no engine object matched ${by ?? "name"}="${value}"`);
  ...
  server.tool(
    "engine_swipe",
    "Swipes between two screen coordinates inside a game-engine view." + INSTRUMENT_NOTE,
    {
      udid: z.string().describe("Device UDID / Android serial"),
      fromX: z.number(),
      fromY: z.number(),
  ...
  server.tool(
    "engine_call",
    "Invokes a C# component method on a game-engine object by reflection — the engine analog of firing a DOM event handler." +
      INSTRUMENT_NOTE,
    {
      udid: z.string().describe("Device UDID / Android serial"),
      by: BY.optional(),
