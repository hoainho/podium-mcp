...
const SAMPLE_TOOLS: ToolDef[] = [
  {
    name: "tap_on",
    description:
      "Taps an element by text or id (resolved via the native element list) or at x/y coordinates; native tap with a Maestro fallback, reports the backend used.",
    schema: { udid: "string", bundleId: "string", text: "string?", id: "string?", x: "number?", y: "number?", double: "boolean?", long: "boolean?" },
  },
  {
    name: "canvas_resolve",
    description:
      "Resolves a fuzzy intent (e.g. close, settings) to a ranked, evidenced canvas target without tapping; returns best, candidates with reasons, and a fail-closed confidentEnough flag.",
    schema: { udid: "string", intent: "string", webviewId: "string?" },
  },
  {
    name: "webview_network",
    description:
      "Captures HTTP traffic made inside a WebView (fetch + XMLHttpRequest) and exports redacted JSON or a HAR 1.2 log; injects an in-page recorder via eval and captures for durationMs.",
...
function structuredStep(n: number): string {
...
export function registerTokenTools(server: McpServer): void {
  server.tool(
    "podium_token_report",
    "Quantifies Podium's token savings: for an N-step flow, computes input tokens for Podium's no-vision " +
    ...
    async ({ steps, screenshotWidth, screenshotHeight, elementsPerStep, toolCount }) => {
      ...
      const noVision: FlowStep[] = Array.from({ length: n }, () => ({ kind: "text", text: structuredStep(els) }));
      const visionLoop: FlowStep[] = Array.from({ length: n }, () => [
        { kind: "image", width: w, height: h } as FlowStep,
        { kind: "text", text: "Look at the screenshot and decide the next tap." } as FlowStep,
      ]).flat();
      ...
      const summary =
        `Podium no-vision vs screenshot/vision loop over ${n} steps: ` +
