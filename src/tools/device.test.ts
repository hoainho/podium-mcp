...
// We capture tool handlers without going through the real MCP transport.
// The handler type from the SDK is (args: Record<string,unknown>) => Promise<{content:...}>
...
describe("simctl helpers", () => {
  ...
  it("setLocation builds correct xcrun simctl location <udid> set <lat>,<lon> args", async () => {
    ...
    const [cmd, args] = spy.mock.calls[0] as [string, string[]];
...
describe("device tools via fake server", () => {
  ...
  async function buildServer() {
    ...
    registerDeviceTools(fake as unknown as import("@modelcontextprotocol/sdk/server/mcp.js").McpServer);
    return fake;
  }
  ...
  it("device_list merges iOS devices and reports android absent when adb is missing", async () => {
    ...
    const payload = JSON.parse(response.content[0].text) as {
      ios: Array<{ udid: string; state: string }>;
      android: { available: boolean; reason: string };
    };
  ...
  it("app_list: parses listApps result (mocked run for simctl + plutil)", async () => {
    ...
    const payload = JSON.parse(response.content[0].text) as {
      count: number;
      apps: Array<{ bundleId: string; name: string; type: string }>;
    };
  ...
  it("orientation_get: returns portrait when height > width", async () => {
    ...
    const payload = JSON.parse(response.content[0].text) as { orientation: string };
...
describe("recording module", () => {
  ...
  it("auto-stops the recording after PODIUM_MAX_RECORDING_MS (watchdog)", async () => {
    ...
    try {
      ...
      await recording.stopRecording(udid).catch(() => undefined);
      if (prev === undefined) delete process.env.PODIUM_MAX_RECORDING_MS;
      else process.env.PODIUM_MAX_RECORDING_MS = prev;
    }
  }, 15_000);
  ...
  it("record_start default path is timestamped and unique across calls", async () => {
    ...
    const p2 = (JSON.parse(r2.content[0].text) as { path: string }).path;
    await recording.stopRecording(udid).catch(() => undefined);
