...
export function registerFlowTools(server: McpServer): void {
  ...
  server.tool(
    "run_flow",
    "Execute one or more Maestro flows on a device. Provide exactly one of: " +
    ...
    async ({ udid, yaml, files, dir, includeTags, excludeTags, env, timeoutMs }) => {
      ...
      if (sourcesProvided.length === 0) {
        return errorResult(
          "run_flow requires exactly one of: yaml, files, or dir — none provided."
        );
      }
      if (sourcesProvided.length > 1) {
        return errorResult(
          "run_flow requires exactly one of: yaml, files, or dir — multiple provided. " +
      ...
      try {
        ...
        if (!result.passed) {
          return errorResult(
            `Flow did not pass (retries: ${result.retries}):\n${result.rawOutput}`
          );
        }
  ...
  server.tool(
    "export_flow",
    "Exports a run_steps action sequence to a reusable Maestro flow (the engineer→QA bridge). " +
  ...
  server.tool(
    "cheat_sheet",
    "Returns the bundled Maestro flow script cheat sheet (offline copy). " +
      "Consult this before authoring unfamiliar Maestro commands, required args, " +
      "nested properties, conditionals, or multi-screen flows.",
    {},
    async () => {
      try {
