...
function elementSignature(
  els: Array<{ label?: string; value?: string; identifier?: string }> | null
): string | null {
  if (!els) return null;
...
export function registerScreenTools(server: McpServer): void {
  ...
  server.tool(
    "inspect_screen",
    "Returns the current view hierarchy for a booted iOS simulator (podium's target platform). " +
    ...
    async ({ udid, bundleId, direction, startX, startY, endX, endY, timeoutMs, noLaunch }) => {
      const overrides =
        startX !== undefined && startY !== undefined && endX !== undefined && endY !== undefined
          ...
          : undefined;
      const g = await nativeSwipe(
        udid,
        { direction, overrides },
        { bundleId, timeoutMs: timeoutMs ?? 30_000, launchApp: !noLaunch }
      );
      if (!g.ok) {
  ...
  server.tool(
    "press_key",
    "Presses a hardware or system key via an ephemeral Maestro flow on the iOS simulator. " +
