...
export function registerAssertTools(server: McpServer): void {
  server.tool(
    "assert_visible",
    "Asserts an element/text is visible, via the oracle ladder (WebView-DOM > native a11y > Maestro). " +
      ...
      "surface) or selector (WebView).",
    { ...target, timeoutMs: z.number().int().min(0).max(120000).optional().describe("Poll budget (default 3000)") },
    async ({ udid, text, selector, bundleId, timeoutMs, contains }) => {
      if (!text && !selector) return errorResult("assert_visible requires text or selector.");
      const r = await checkVisible(udid, { text, selector }, { timeoutMs, bundleId, contains });
      if (r.visible === true) return okResult({ ok: true, visible: true, via: r.via, ...(text ? { text } : {}), ...(selector ? { selector } : {}) });
      if (r.visible === null)
  ...
  server.tool(
    "assert_text",
    "Asserts the given text is visible on screen (by-text shorthand for assert_visible). Same oracle ladder + " +
      "unverifiable handling.",
    { udid: target.udid, text: z.string().describe("Visible text to assert"), bundleId: target.bundleId, contains: target.contains, timeoutMs: z.number().int().min(0).max(120000).optional() },
    async ({ udid, text, bundleId, timeoutMs, contains }) => {
      const r = await checkVisible(udid, { text }, { timeoutMs, bundleId, contains });
      if (r.visible === true) return okResult({ ok: true, visible: true, via: r.via, text });
      if (r.visible === null) return errorResult(`assert_text: UNVERIFIABLE (${r.via}) — could not read the target (WebView not inspectable?).`);
  ...
  server.tool(
    "assert_not_visible",
    "Asserts an element/text is ABSENT. FAILS CLOSED: if absence cannot be verified (e.g. a WebView whose DOM " +
      "is unreadable — native a11y is blind to web content), returns an 'unverifiable' error rather than a false " +
      "pass. Passes only when a capable oracle confirms absence.",
    { ...target, timeoutMs: z.number().int().min(0).max(120000).optional().describe("Confirmation budget (default 1500)") },
    async ({ udid, text, selector, bundleId, timeoutMs, contains }) => {
      if (!text && !selector) return errorResult("assert_not_visible requires text or selector.");
      const r = await checkVisible(udid, { text, selector }, { timeoutMs: timeoutMs ?? 1500, bundleId, contains });
      if (r.visible === false) return okResult({ ok: true, notVisible: true, via: r.via, ...(text ? { text } : {}), ...(selector ? { selector } : {}) });
      if (r.visible === null)
  ...
  server.tool(
    "wait_for_element",
    "Polls until an element/text is visible (via the oracle ladder), or fails on timeout. Use to act the instant " +
      "the UI is ready instead of a blind sleep.",
    { ...target, timeoutMs: z.number().int().min(0).max(120000).optional().describe("Wait budget (default 10000)") },
    async ({ udid, text, selector, bundleId, timeoutMs, contains }) => {
      if (!text && !selector) return errorResult("wait_for_element requires text or selector.");
      const budget = timeoutMs ?? 10000;
      const r = await checkVisible(udid, { text, selector }, { timeoutMs: budget, bundleId, contains });
      if (r.visible === true) return okResult({ ok: true, visible: true, via: r.via });
      if (r.visible === null) return errorResult(`wait_for_element: UNVERIFIABLE (${r.via}) — could not read the target (WebView not inspectable?).`);
