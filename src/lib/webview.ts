...
const NO_BACKEND =
  "webview tools require the bundled mobilecli backend, which did not resolve. " +
...
function parseEnvelope<T>(stdout: string): T | null {
  try {
    const p = JSON.parse(stdout) as { status?: string; data?: T };
...
export async function listWebviews(udid: string): Promise<Result<Webview[]>> {
  const bin = await resolveMobilecli();
  if (!bin) return { ok: false, error: NO_BACKEND };
  ...
  try {
    const p = JSON.parse(raw) as { status?: string; data?: Webview[]; error?: string };
  ...
  if (r.code !== 0 && raw) return { ok: false, error: webviewError(raw.slice(0, 300)) };
  if (!raw) {
    return {
      ok: false,
      error:
        "webview list returned no output — mobilecli's WebView backend (WebDriverAgent/DeviceKit) " +
...
function webviewError(msg: string): string {
 ...
 */
export async function resolveWebview(
  udid: string,
  webviewId?: string
): Promise<Result<Webview>> {
  ...
  if (list.data.length === 0) {
    return {
      ok: false,
      error:
        "no inspectable WebView found on this device. Likely causes: (1) the app hosts no " +
 ...
 */
export async function evalWebview(
  udid: string,
  webviewId: string,
  expression: string
  ...
  const data = parseEnvelope<string>(r.stdout);
...
export interface DomElement {
  ...
  /** Absolute logical-point screen coordinates (ready for tap_on). */
  tapX: number;
  tapY: number;
}
 ...
 */
export async function inspectWebview(
  udid: string,
  selector: string,
  webviewId: string | undefined,
  max = 100
): Promise<Result<{ webview: Webview; count: number; elements: DomElement[]; truncated: boolean }>> {
  ...
  const expr =
    `JSON.stringify([].slice.call(document.querySelectorAll(${sel})).map(function(e){` +
  ...
  let raw: Array<{ tag: string; id: string | null; text: string; rect: { x: number; y: number; w: number; h: number } }>;
  try {
    raw = JSON.parse(ev.data) as typeof raw;
  } catch {
  ...
  const elements: DomElement[] = raw.slice(0, max).map((e) => ({
    tag: e.tag,
    id: e.id,
    text: e.text,
    rect: e.rect,
...
export async function navigateWebview(
  ...
  url?: string
): Promise<Result<{ action: string; url?: string }>> {
  const bin = await resolveMobilecli();
  if (!bin) return { ok: false, error: NO_BACKEND };

  const args = ["webview", action, webviewId];
  if (action === "goto") {
    if (!url) return { ok: false, error: "webview_navigate: action 'goto' requires a url." };
    args.push(url);
  }
...
/** Compact record shape emitted by the shim (short keys to keep eval payloads small). */
export interface WebviewNetRecord {
  u?: string;
  m?: string;
...
export function mapWebviewNetRecords(recs: WebviewNetRecord[]): NetworkEntry[] {
  return recs.map((r, i) => ({
    requestId: String(i),
    url: r.u || "",
    method: r.m || "GET",
...
const WEBVIEW_RESOURCE_READ =
  "JSON.stringify(((typeof performance!=='undefined'&&performance.getEntriesByType)?performance.getEntriesByType('resource'):[]).map(function(e){" +
...
/** Compact record from the Performance Resource Timing buffer. */
export interface WebviewResRecord {
  u?: string;
  it?: string;
  d?: number;
  sz?: number;
...
export function mapResourceRecords(recs: WebviewResRecord[]): NetworkEntry[] {
  return recs
    .filter((r) => r.u)
    .map((r, i) => ({
      requestId: "res-" + i,
...
export function mergeWebviewNetwork(xhr: WebviewNetRecord[], resources: WebviewResRecord[]): NetworkEntry[] {
...
export async function webviewNetworkCapture(
  udid: string,
  webviewId: string,
  durationMs: number,
  opts: { includeResources?: boolean } = {}
): Promise<Result<NetworkEntry[]>> {
  ...
  let recs: WebviewNetRecord[];
  try {
    recs = JSON.parse(read.data) as WebviewNetRecord[];
  } catch {
    return { ok: false, error: `webview network buffer was not JSON: ${read.data.slice(0, 200)}` };
  }
  if (!Array.isArray(recs)) return { ok: false, error: "webview network buffer was not an array" };
  ...
  let resources: WebviewResRecord[] = [];
  if (resRead.ok) {
    try {
      const parsed = JSON.parse(resRead.data) as WebviewResRecord[];
