/**
 * v0.4.0 "Canvas Brain" — no-vision framework adapters (story 2).
 *
 * A `<canvas>` is opaque to the DOM, but most JS canvas/WebGL games keep a
 * live scene graph in memory (Pixi/Konva/Fabric/Phaser/Three/Babylon). This
 * module gives us DOM-like, ZERO-screenshot addressing of those nodes:
 *
 *  - `buildCanvasBridgeScript()` returns a self-contained IIFE (one JS string)
 *    that, eval'd in the page, installs `window.__podiumCanvas` — auto-detecting
 *    the framework, walking its scene graph, and reporting each node's geometry
 *    CONVERTED to CSS px relative to the canvas element top-left (the unit
 *    `tap_on` expects). The conversion handles devicePixelRatio / backing-store
 *    size by scaling backing-store px by `getBoundingClientRect().width /
 *    canvas.width`.
 *  - `parseCanvasObjects()` is the TS-side normalizer (mirrors
 *    engine.ts#parseEngineObjects): it turns a bridge dump — value or JSON
 *    string — into typed `CanvasObject[]`, dropping anything that isn't tappable
 *    (no finite x/y) and stamping `source: "scene-graph"`.
 *
 * The bridge body is authored as a plain JS string (the same approach
 * engine.ts#createWebviewEngineTransport uses for its eval expression) so there
 * is no TS→JS transpile step hidden inside a string at runtime. It can't run in
 * CI (no browser), so it is validated by the contract tests (parses as valid JS
 * via `new Function`, contains the detection tokens) and by live e2e later.
 * Everything is fail-closed: the bridge never throws across the eval boundary —
 * it returns `{framework:"unknown", objects:[]}` on any error.
 */
import type { CanvasObject, CanvasRect, CanvasFramework } from "./canvas-types.js";

function isCanvasFramework(value: unknown): value is CanvasFramework {
  return (
    value === "konva" ||
    value === "fabric" ||
    value === "pixi" ||
 ...
 */
export function parseCanvasObjects(raw: unknown): CanvasObject[] {
  let arr: unknown = raw;
  ...
  return arr.flatMap((o): CanvasObject[] => {
    ...
    const obj: CanvasObject = {
      ...
      x,
      y,
      ...(idIsString ? { id: rec.id as string } : idIsNumber ? { id: rec.id as number } : {}),
 ...
 */
export function detectFrameworkExpression(): string {
 ...
 */
export function buildCanvasBridgeScript(): string {
  return (
    ";(function(){try{" +
    BRIDGE_BODY +
