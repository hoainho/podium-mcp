...
describe("listAllTargets", () => {
  ...
  it("merges targets across all registered drivers", async () => {
    registerDriver(
      stubDriver("ios-sim", [{ udid: "SIM-1", platform: "ios-sim", name: "iPhone 16 Pro" }])
    );
    registerDriver(
      stubDriver("android", [{ udid: "emulator-5554", platform: "android", name: "Pixel 8" }])
    );
  ...
  it("degrades: a driver whose list() throws contributes nothing but does not abort", async () => {
    registerDriver(
      stubDriver("ios-sim", [{ udid: "SIM-1", platform: "ios-sim" }])
    );
