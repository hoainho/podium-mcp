...
describe("result envelope (model-agnostic contract / G003)", () => {
  it("okResult injects status:'ok' and mirrors to structuredContent", () => {
    ...
    expect((r as { isError?: true }).isError).toBeUndefined();
  });
  ...
  it("errorResult maps a known code into the machine-readable status", () => {
    expect((errorResult({ code: "ambiguous", message: "two matches" }).structuredContent as { status: string }).status).toBe("ambiguous");
    expect((errorResult({ code: "weird", message: "x" }).structuredContent as { status: string }).status).toBe("failed");
  });
});
