...
import { parseWdaSource, makeWdaBackend, type FetchLike } from "./wda.js";
...
describe("makeWdaBackend", () => {
  ...
  it("describeAll GETs /source and parses it", async () => {
    const { fetchImpl, urls } = mockFetch((url) =>
      url.includes("/source") ? { ok: true, status: 200, body: SOURCE } : { ok: true, status: 200, body: "{}" }
    );
...
describe("getBackendFor ios-real WDA opt-in", () => {
  ...
  afterEach(() => {
    if (prev === undefined) delete process.env.PODIUM_WDA_URL;
    else process.env.PODIUM_WDA_URL = prev;
  });
