 ...
 */
export function parseUiautomatorXml(xml: string): NativeElement[] {
  const els: NativeElement[] = [];
  ...
  while ((m = nodeRe.exec(xml)) !== null) {
    ...
    if (!label && !resId) continue;
    const el: NativeElement = { label };
    if (resId) el.identifier = resId;
    if (cls) el.type = cls;
...
export function makeAdbBackend(): NativeBackend {
  return {
    name: "adb",
    tap: (serial, x, y) =>
      run(ADB, ["-s", serial, "shell", "input", "tap", px(x), px(y)], { timeout: 15_000 }),
    swipe: (serial, x1, y1, x2, y2, durationMs) => {
      const args = ["-s", serial, "shell", "input", "swipe", px(x1), px(y1), px(x2), px(y2)];
      if (durationMs && durationMs > 0) args.push(px(durationMs));
      return run(ADB, args, { timeout: 15_000 });
    },
    inputText: (serial, text) =>
      run(ADB, ["-s", serial, "shell", "input", "text", escapeInputText(text)], { timeout: 15_000 }),
