 ...
 */
export function parseAdbDevices(stdout: string): DeviceTarget[] {
  return stdout
    .split("\n")
    .slice(1)
    .map((l) => l.trim())
    ...
    .map((line) => {
      ...
      const transport: DeviceTarget["transport"] = serial.includes(":") ? "network" : "usb";
      return { udid: serial, platform: "android" as const, name, state, transport };
    });
}
 ...
 */
export function parseWmSize(stdout: string): { widthPx: number; heightPx: number } | null {
...
export const androidDriver: PlatformDriver = {
  platform: "android",
  list: listAndroidTargets,
  // Android devices/emulators are already running; there is no per-device boot.
  install: async (serial, apkPath) =>
    toResult(await run(ADB, ["-s", serial, "install", "-r", apkPath], { timeout: 120_000 })),
  launch: async (serial, pkg) =>
    toResult(
      await run(
        ADB,
  ...
  terminate: async (serial, pkg) =>
    toResult(await run(ADB, ["-s", serial, "shell", "am", "force-stop", pkg], { timeout: 15_000 })),
