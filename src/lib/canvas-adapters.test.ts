...
import {
  ...
  detectFrameworkExpression,
} from "./canvas-adapters.js";
...
describe("parseCanvasObjects", () => {
  ...
  it("accepts a JSON-string payload (bridge may double-encode)", () => {
    expect(
      parseCanvasObjects(JSON.stringify([{ name: "n", x: 7, y: 8, framework: "phaser" }]))
    ).toEqual([{ name: "n", x: 7, y: 8, framework: "phaser", source: "scene-graph" }]);
  });
