...
describe("compareFlows", () => {
  it("savingsRatio > 1 when the vision loop consumes more tokens than the structured flow", () => {
    ...
    const noVision: FlowStep[] = Array.from({ length: 8 }, () => ({
      kind: "text" as const,
    ...
    const visionLoop: FlowStep[] = Array.from({ length: 8 }, () => ({
      kind: "image" as const,
      width: 1170,
      height: 2532,
    }));
  ...
  it("no divide-by-zero when noVision is empty but visionLoop is not", () => {
    const visionLoop: FlowStep[] = [{ kind: "image", width: 100, height: 100 }];
  ...
  it("breakdown fields are consistent with totals", () => {
    const noVision: FlowStep[] = [{ kind: "text", text: "hello world" }];
    const visionLoop: FlowStep[] = [
    ...
    expect(result.visionTokens).toBe(
      result.breakdown.visionTextTokens + result.breakdown.visionImageTokens
    );
