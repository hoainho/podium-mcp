/**
 * Real-iOS gesture/inspection backend via WebDriverAgent (v0.3.0 story B2).
 *
 * WebDriverAgent (WDA) is an XCUITest HTTP server running on the device; it
 * exposes the accessibility tree (`GET /source`) and gestures (tap/swipe/typed
 * text) over HTTP. This backend implements the NativeBackend contract against a
 * WDA base URL, so tap_on/inspect_screen work on a real iPhone exactly like the
 * iOS-sim path.
 *
 * The HTTP layer is injectable (FetchLike) so the request-building / source-parse
 * logic is unit-tested with a mock. The exact WDA endpoint shapes and a live
 * session handshake are validated on hardware (story B3) — until then this is
 * opt-in via PODIUM_WDA_URL and fails closed (every method degrades to a
 * non-zero RunResult / null) when the server is unreachable.
 *
 * NativeBackend/NativeElement are imported type-only to avoid a runtime cycle.
 */
import type { RunResult } from "./exec.js";
import type { NativeBackend, NativeElement } from "./native.js";
 ...
 */
export function parseWdaSource(json: string): NativeElement[] {
  let root: unknown;
  try {
...
const WDA_KEY: Record<string, string> = {
  home: "home",
  enter: "\n",
  return: "\n",
};
 ...
 */
export function makeWdaBackend(baseUrl: string, fetchImpl: FetchLike = fetch): NativeBackend {
  const base = baseUrl.replace(/\/$/, "");
  return {
    name: "wda",
    tap: (_udid, x, y) => wdaRequest(fetchImpl, `${base}/wda/tap/0`, "POST", { x, y }),
    swipe: (_udid, x1, y1, x2, y2, durationMs) =>
      wdaRequest(fetchImpl, `${base}/wda/dragfromtoforduration`, "POST", {
        fromX: x1,
        fromY: y1,
        toX: x2,
        toY: y2,
    ...
    pressKey: async (_udid, key) => {
      ...
      if (mapped === "home") return wdaRequest(fetchImpl, `${base}/wda/homescreen`, "POST");
