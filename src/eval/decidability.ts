...
export function evaluateDecidability(res: ToolResultLike): Decision {
  const sc = asRecord(res.structuredContent);
  if (!sc) return { decidable: false, status: null, reason: "no structuredContent (model must parse prose)" };
  const status = typeof sc["status"] === "string" ? (sc["status"] as string) : null;
  if (!status) return { decidable: false, status: null, reason: "no machine-readable status" };

  if (!res.isError) {
    if (status === "ok") return { decidable: true, status, reason: "success: status=ok" };
  ...
  // hard error
  if (!NON_OK.has(status)) return { decidable: false, status, reason: `error status=${status} not in known set` };
  const err = asRecord(sc["error"]);
  const actionable = !!err && (err["remediation"] != null || err["suggestedTool"] != null ||
    (Array.isArray(err["candidates"]) && (err["candidates"] as unknown[]).length > 0));
  return actionable
