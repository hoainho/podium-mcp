...
function nodeToCanvasObject(node) {
    ...
    const text = typeof node.label === "string" && node.label.length > 0
        ? node.label
 ...
 */
export function parseA11yTree(domJson) {
    try {
        let raw = domJson;
        ...
        for (const node of raw) {
            if (node == null || typeof node !== "object")
                continue;
            const obj = nodeToCanvasObject(node);
            if (obj !== null)
                objects.push(obj);
        }
        return objects;
 ...
 */
export function buildA11ySnapshotScript() {
