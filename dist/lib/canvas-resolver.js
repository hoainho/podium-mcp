 ...
 */
function topRightScore(obj, surface) {
 ...
 */
export function scoreObject(obj, intent, surface) {
    const reasons = [];
    ...
    if (score > 0) {
        const interactable = obj.interactable === true || (obj.role ? INTERACTABLE_ROLES.has(role) : false);
        if (interactable) {
            add(W.interactable);
 ...
 */
export function resolveIntent(objects, intent, opts) {
    ...
    const confidentEnough = best !== null &&
