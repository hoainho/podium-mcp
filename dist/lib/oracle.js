 ...
 */
export async function pollVisible(udid, be, text, timeoutMs, bundleId) {
 ...
 */
export function targetingHint(surface) {
 ...
 */
export async function detectSurface(udid) {
...
async function domVisible(udid, webviewId, sel) {
 ...
 */
export async function checkVisible(udid, sel, opts = {}) {
    ...
    for (;;) {
        if (surface.surface === "webview" && surface.webviewId) {
            ...
            if (dom === null)
                return { visible: null, via: "unverifiable" }; // can't read DOM → don't guess
            if (dom === true)
                return { visible: true, via: "webview-dom" };
            if (Date.now() - start >= timeoutMs)
                return { visible: false, via: "webview-dom" };
        }
        else {
            ...
            if (sel.text) {
                ...
                if (be) {
                    ...
                    if (els) {
                        ...
                        if (hit)
                            return { visible: true, via: "native-a11y" };
                    }
                }
                else {
                    ...
                    if (appId) {
                        ...
                        if (m?.passed)
                            return { visible: true, via: "maestro" };
                    }
                }
                if (Date.now() - start >= timeoutMs)
                    return { visible: false, via: "native-a11y" };
            }
            else {
