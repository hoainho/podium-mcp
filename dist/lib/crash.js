...
export const DEFAULT_DIAGNOSTICS_DIR = join(os.homedir(), "Library", "Logs", "DiagnosticReports");
/** Per-simulator DiagnosticReports dir — sim-process crashes can land here instead of the host dir. */
export function simDiagnosticsDir(udid) {
    return join(os.homedir(), "Library", "Developer", "CoreSimulator", "Devices", udid, "data", "Library", "Logs", "DiagnosticReports");
}
/**
 ...
 */
export async function listCrashes(filter, dir = DEFAULT_DIAGNOSTICS_DIR) {
    const dirs = [
    ...
    const results = [];
    await Promise.all(dirs.map(async ({ path: scanDir, source }) => {
        let entries;
        try {
        ...
        const relevant = entries.filter((f) => f.endsWith(".ips") || f.endsWith(".crash"));
        await Promise.all(relevant.map(async (filename) => {
 ...
 */
function extractProcessName(filename) {
    // Strip extension
    ...
    if (firstNumericIdx > 0)
        return parts.slice(0, firstNumericIdx).join("-");
    return noExt;
}
// ─── crash_get ────────────────────────────────────────────────────────────────
 ...
 */
export async function getCrash(id, dir = DEFAULT_DIAGNOSTICS_DIR, udid) {
