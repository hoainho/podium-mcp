...
import { EngineClient, EngineError, parseEngineObjects, forwardEnginePort, createWebviewEngineTransport, } from "./engine.js";
...
describe("parseEngineObjects", () => {
    it("parses an array of objects carrying screen coords", () => {
        expect(parseEngineObjects([{ name: "PlayButton", id: 5, x: 540, y: 1200, enabled: true, type: "Button" }])).toEqual([{ name: "PlayButton", id: 5, x: 540, y: 1200, enabled: true, type: "Button" }]);
    });
...
describe("EngineClient", () => {
    ...
    it("callComponentMethod forwards component/method/parameters (DOM-event analog)", async () => {
        ...
        const out = await new EngineClient(transport).callComponentMethod({ name: "B", id: 7, x: 1, y: 2 }, "ScoreController", "AddPoints", [10]);
...
describe("forwardEnginePort", () => {
    ...
    it("Android forwards via adb", async () => {
        ...
        expect(spy).toHaveBeenCalledWith("adb", ["-s", "emulator-5554", "forward", "tcp:13000", "tcp:13000"], expect.anything());
    });
...
describe("createWebviewEngineTransport (WebGL-in-WebView)", () => {
    ...
    it("fails closed (EngineError) when the eval itself throws (no bridge present)", async () => {
        const client = new EngineClient(createWebviewEngineTransport(async () => {
