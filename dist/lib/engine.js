 ...
 */
export async function forwardEnginePort(platform, udid, port = ENGINE_DEFAULT_PORT) {
    if (platform === "android") {
 ...
 */
export function parseEngineObjects(data) {
    let arr = data;
...
export class EngineClient {
    ...
    async callComponentMethod(obj, component, method, parameters = []) {
 ...
 */
export function createWebviewEngineTransport(evalJs) {
    return {
        async send(command) {
            const cmdJson = JSON.stringify(command);
            const expr = `JSON.stringify((window.__podiumEngine||function(){throw new Error("no __podiumEngine bridge on this WebGL page")})(${JSON.stringify(cmdJson)}))`;
            let raw;
            try {
