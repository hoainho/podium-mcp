...
function flowHeader(appId, launchApp) {
 ...
 */
export async function nativeKey(udid, key, opts) {
    const be = await getBackend();
    if (be && be.canPressKey(key)) {
        const r = await be.pressKey(udid, key);
        if (r && r.code === 0)
            return { ok: true, backend: be.name };
    }
 ...
 */
export async function nativeInputText(udid, text, opts) {
 ...
 */
export async function nativeSwipe(udid, spec, opts) {
    const be = await getBackend();
    if (be) {
        ...
        if (dims) {
            ...
            if (r.code === 0)
                return { ok: true, backend: be.name, points: pts };
 ...
 */
export async function nativeTapText(udid, sel, opts) {
    if (!sel.text && !sel.id) {
    ...
    if (be) {
        ...
        if (els) {
            ...
            if (pt) {
                ...
                if (r.code === 0)
                    return { ok: true, backend: be.name, tappedAt: pt };
            }
        }
    }
    const appId = await appIdFor(udid, opts?.bundleId);
    if (!appId) {
        return {
            ok: false,
            backend: "maestro",
            detail: "element not found natively and no appId for Maestro fallback — pass bundleId or use a coordinate tap",
        };
    }
    const selector = sel.text && sel.id
 ...
 */
export async function resolveForegroundApp(udid) {
 ...
 */
export async function nativeTap(udid, x, y, opts) {
    const be = await getBackend();
    if (be) {
    ...
    if (!appId) {
        return {
            ok: false,
            backend: "maestro",
            detail: "no tap backend available: neither idb nor mobilecli resolved, and no " +
