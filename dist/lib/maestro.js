 ...
 */
export async function resolveMaestro() {
...
export async function resolveJavaHome() {
    if (cachedJavaHome !== undefined)
        return cachedJavaHome ?? undefined;
...
export async function maestroEnv() {
 ...
 * Never throws.
 */
export async function proactiveIdbRestart() {
 ...
 */
const IDB_FLAKY_RE = /Failed to connect to 127\.0\.0\.1|java\.net\.ConnectException|idb_companion/i;
/** Truncate a string to approximately `maxChars` from the tail. */
function tailTruncate(s, maxChars = 4000) {
...
export async function runMaestroFlow(opts) {
    const { udid, yaml, files, dir, includeTags, excludeTags, env, timeoutMs } = opts;
    // Validate exactly-one-of
    const sourcesProvided = [yaml, files, dir].filter((v) => v !== undefined);
    if (sourcesProvided.length !== 1) {
        throw new Error(`runMaestroFlow requires exactly one of yaml, files, or dir — got ${sourcesProvided.length === 0 ? "none" : "multiple"}.`);
    }
    const binary = await resolveMaestro();
    if (!binary) {
        throw new Error("maestro binary not found. Install Maestro: https://maestro.mobile.dev");
    }
 ...
 */
export async function getHierarchy(udid) {
    const binary = await resolveMaestro();
    if (!binary) {
        return {
            ok: false,
    ...
    if (/unknown command|invalid command|No such subcommand|hierarchy.*not.*found/i.test(combined) ||
        (result.code !== 0 &&
        ...
        return {
            ok: false,
            unsupported: true,
            text: "This Maestro version does not expose a `hierarchy` CLI command. " +
    ...
    if (firstJson !== -1) {
        try {
            const parsed = JSON.parse(result.stdout.slice(firstJson));
