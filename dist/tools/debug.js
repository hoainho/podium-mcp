 ...
 */
async function resolveMetroWs(webSocketDebuggerUrl, port) {
    if (webSocketDebuggerUrl)
        return { wsUrl: webSocketDebuggerUrl };
    const apps = await listMetroApps(port ?? 8081);
    if ("error" in apps)
        return { error: apps.error };
    if (apps.length === 0)
        return { error: "metro_apps found no connected apps" };
    const first = apps[0];
...
export function registerDebugTools(server) {
    ...
    // ─── metro_logs ──────────────────────────────────────────────────────────────
    server.tool("metro_logs", "Reads console logs from a React Native app via the Metro CDP debugger. If webSocketDebuggerUrl is omitted, auto-discovers via metro_apps and uses the first connected app. Reports which app was chosen. Pass saveTo to also write the console timeline to a file for evidence (e.g. attach to a bug).", {
    ...
    // ─── metro_network ─────────────────────────────────────────────────────────
    server.tool("metro_network", "Captures network requests from a React Native app via the Metro CDP debugger (Network domain). " +
    ...
    // ─── metro_state ─────────────────────────────────────────────────────────────
    server.tool("metro_state", "Reads app state from a React Native app by evaluating a JS expression in its runtime via the " +
        ...
        expression: z
            .string()
            .optional()
            .describe("JS expression to evaluate (default: a globally-exposed Redux store's getState()). " +
    ...
    // ─── app_state ───────────────────────────────────────────────────────────────
    server.tool("app_state", "Checks whether an app is installed and/or running on an iOS simulator. installed: exact " +
        ...
        const running = launchctlResult.code === 0 &&
            new RegExp(`UIKitApplication:${escapeRegExp(bundleId)}(?:\\[|\\s|$)`, "m").test(launchctlResult.stdout);
