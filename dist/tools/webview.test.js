...
describe("webview_network capture", () => {
    ...
    it("mergeWebviewNetwork keeps rich fetch/XHR records and adds non-duplicate resources", () => {
        const merged = mergeWebviewNetwork([{ u: "https://x/api/balance", m: "GET", s: 200, q: { Authorization: "secret" } }], [
    ...
    function mockMobilecli(records = RECORDS) {
        vi.spyOn(nativeLib, "resolveMobilecli").mockResolvedValue("/fake/mobilecli");
        vi.spyOn(exec, "run").mockImplementation(async (_bin, args) => {
            if (args[1] === "list") {
                return {
                    code: 0,
    ...
    it("captures fetch/XHR records and emits a redacted HAR by default", async () => {
        ...
        expect(entries).toHaveLength(2);
        const authHdr = entries[0].request.headers.find((h) => h.name === "Authorization");
        expect(authHdr.value).toBe("***REDACTED***");
        const setCookie = entries[0].response.headers.find((h) => h.name === "Set-Cookie");
        expect(setCookie.value).toBe("***REDACTED***");
        expect(entries[1].request.postData.text).toBe("***REDACTED***");
