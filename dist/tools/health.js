...
export function registerHealthTool(server) {
    server.tool(TOOL_NAME, "Returns health status of the podium-mcp server and toolchain availability. " +
        "Scope: iOS (simulator + real device), Android (emulator + real via adb), game-engine " +
        "(Unity/GL via AltTester), and no-vision canvas/WebGL automation. macOS + Xcode required; adb " +
        "for Android, an instrumented build for engine tools.", {}, async () => {
