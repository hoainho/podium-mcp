...
function makeFakeBackend(overrides = {}) {
    ...
    const base = {
        ...
        canPressKey: (k) => ["home", "lock", "power", "volume up", "volume down"].includes(k),
...
async function buildServer() {
    ...
    registerStepsTools(fake);
    return fake;
}
async function callRunSteps(args) {
...
describe("run_steps", () => {
    ...
    it("waitFor fails fast when the text never appears", async () => {
        vi.spyOn(nativeLib, "getBackend").mockResolvedValue(makeFakeBackend({ describeAll: vi.fn(async () => [{ label: "Other" }]) }));
        const payload = await callRunSteps({
            udid: "UDID",
