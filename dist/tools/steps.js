...
import { nativeTap, nativeKey, nativeInputText, nativeSwipe, nativeTapText, } from "../lib/gesture.js";
...
async function execStep(udid, bundleId, be, step, i) {
    switch (step.action) {
        ...
        case "tapText": {
            ...
            const res = { i, action: "tapText", ok: g.ok, backend: g.backend };
            if (g.tappedAt)
                res.tappedAt = g.tappedAt;
            if (g.selector)
                res.selector = g.selector;
            if (!g.ok && g.detail)
                res.error = g.detail;
            return res;
        }
        case "type": {
            ...
            const res = { i, action: "type", ok: g.ok, backend: g.backend };
            if (g.submit !== undefined)
                res.submit = g.submit;
            if (g.ok && g.detail)
                res.detail = g.detail;
            if (!g.ok && g.detail)
        ...
        case "key": {
            ...
            const res = { i, action: "key", ok: g.ok, backend: g.backend, key: step.key };
            if (!g.ok && g.detail)
                res.error = g.detail;
            return res;
        }
        case "swipe": {
            const points = step.startX !== undefined &&
            ...
            const res = { i, action: "swipe", ok: g.ok, backend: g.backend };
            if (g.points)
                res.points = g.points;
            if (!g.ok && g.detail)
                res.error = g.detail;
